...
export function ConsistencySummary({ timestamps, compact = false }: Readonly<{ timestamps: string[]; compact?: boolean }>) {
  ...
  if (compact) return <div className="mt-6 flex gap-3 text-sm"><span className="rounded-full bg-orange-400/10 px-3 py-2 text-orange-200">🔥 {streak}-day streak</span><span className="rounded-full bg-slate-800 px-3 py-2 text-slate-300">{thisMonth} this month</span></div>;
  const gymDays = new Set(timestamps.map((value) => localDateKey(value, timeZone))).size;
  return <div className="grid grid-cols-2 gap-3"><Stat label="Gym days" value={String(gymDays)} /><Stat label="Current streak" value={`${streak} days`} /></div>;
}
