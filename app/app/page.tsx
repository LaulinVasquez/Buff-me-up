...
export default async function ApplicationHome({ searchParams }: Props) {
  ...
  return <main className="min-h-[calc(100dvh-9rem)] py-10 pb-28">
    ...
    <p className="mt-3 text-sm font-semibold text-slate-500">Today</p>
    ...
    {selection.currentWorkout ? <section className="mt-8 rounded-3xl border border-lime-400/30 bg-lime-400/10 p-6">
      ...
      <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Today&apos;s workout</p><h2 className="mt-2 text-2xl font-bold">{selection.day.name}</h2></div><span className="text-sm text-slate-400">{exercises.length} exercises</span></div>
