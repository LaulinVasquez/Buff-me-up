...
export default async function GeneratePlanPage() {
  const [remote, plans] = await Promise.all([searchCatalog({ limit: 50 }), getPlans().catch(() => [])]);
  const merged = [...fallbackExercises, ...remote.exercises]; const pool = [...new Map(merged.map((exercise) => [exercise.id, exercise])).values()];
